...
std::string GarblerClient::run(std::vector<int> input)
{
  ...
  {
    auto m0 = labels.zeros[i + this->circuit.garbler_input_length].value;
    auto m1 = labels.ones[i + this->circuit.garbler_input_length].value;
    this->ot_driver->OT_send(byteblock_to_string(m0), byteblock_to_string(m1));
  }
